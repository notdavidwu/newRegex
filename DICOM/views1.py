...
sys.path.append("C:/Users/T25321/PycharmProjects/AIC/函式庫")
from readDicom import readCT, readPET
from Window import Window_FOR_VIEW, Window
import pathlib
from cv2 import applyColorMap, addWeighted, line, circle
import cv2
...
import h5py
import matplotlib
sys.path.append("C:/Users/T25321/PycharmProjects/django")
...
def load_DICOM(request):
    ...
    with h5py.File(paths, "r") as f:
        ...
        request.session['CT_slicethickness_' + WindowNo] = float(
            pydicom.dataset.Dataset.from_json(headers[0]).SliceThickness)
    ...
    with h5py.File(paths, "r") as f:
        ...
        request.session['PET_pixelspacing_' + WindowNo] = float(
            pydicom.dataset.Dataset.from_json(headers[0]).PixelSpacing[0])
        request.session['PET_slicethickness_' + WindowNo] = float(
            pydicom.dataset.Dataset.from_json(headers[0]).SliceThickness)
...
def load_RT_DICOM(request):
    ...
    with h5py.File(paths, "r") as f:
        ...
        request.session['CT_view_pixelspacing_' + WindowNo] = float(
            pydicom.dataset.Dataset.from_json(headers[0]).PixelSpacing[0])
        request.session['CT_view_slicethickness_' + WindowNo] = float(
            pydicom.dataset.Dataset.from_json(headers[0]).SliceThickness)
        request.session['CT_view_ImagePositionPatient_F_' + WindowNo] = float(
            pydicom.dataset.Dataset.from_json(headers[0]).ImagePositionPatient[2])
        request.session['CT_view_ImagePositionPatient_S_' + WindowNo] = float(
...
def slicing(plane, img_path, variable, drawActualCoordinates, WC, WW, y, tag):
    if variable is None:
        variable = 0
    if plane == 'Axial':
        img = Window_FOR_VIEW(np.array(h5py.File(img_path, "r")['vol'][variable, :, :]), WC, WW)
        z = tag['D']
    elif plane == 'Coronal':
...
def processing_CT(CT, variable, CTWC, CTWW, plane, x, y, height, drawActualCoordinates, CT_tag,contour_list):
    ...
    CTpil_image = applyColorMap(CTimg.astype(np.uint8), cmapy.cmap('gray'))
    CTpil_image = draw_line(CTpil_image, x, y, plane, height)
    if len(contour_list)!=0:
        CTpil_image=segmentation(CTpil_image,contour_list,variable)
    CTpil_image = to_image(CTpil_image)
    CTimage_uri = to_data_uri(CTpil_image)
    dicom_url = CTimage_uri
    return dicom_url, z, ratio

def segmentation(img,contour_list,variable):
    for filepath in contour_list:
...
def processing_PET(CT_view, PET_view, variable, CTWC, CTWW, PETWC, PETWW, plane, mode, x, y, height,
    ...
    CTpil_image = applyColorMap(CTimg.astype(np.uint8), cmapy.cmap('gray'))
    PETpil_image = applyColorMap(PETimg.astype(np.uint8), cmapy.cmap('binary'))
    if mode == 'CT':
        CTpil_image = draw_line(CTpil_image, x, y, plane, height)
        CTpil_image = to_image(CTpil_image)
        CTimage_uri = to_data_uri(CTpil_image)
        ...
        CTpil_image = (CTpil_image[:, :, :3]).astype(np.uint8)
        PET2Fusion = PETimg.astype(np.uint8)
        Fusionimg = applyColorMap(PET2Fusion, GE)
...
def DICOM_show1(request):
    ...
    return JsonResponse({"dicom_url": dicom_url, 'z': z, 'ratio': ratio, 'variable': variable})
...
def insertLocation(request):
    ...
    PID = 'null' if (str(request.POST.get('PID')) == '') else fernet.decrypt(request.POST.get('PID').encode()).decode()
    string = request.POST.get('str').split(',')
    query = '''select　* from Localization where  PID=%s and username=%s or username='' and Disease=%s and StudyID in (%s,%s,%s,%s)'''
    cursor = connections['default'].cursor()
...
def selectLocation(request):
    ...
    query = '''select　* from Localization where  PID=%s and username=%s or username='' and Disease=%s and StudyID in (%s,%s,%s,%s) order by date,LabelName'''
    cursor = connections['default'].cursor()
...
def UNet(request):
    ...
    unet_contour_path = img_path.replace(img_path.split('\\')[-1],'')+id+"_unet.h5"
    contour_list=request.session.get('unet_contour_'+WindowNo)
    if id not in contour_list : 
